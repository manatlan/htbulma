...
class Input(Tag.input, SelfProperties, TagBulma):
    def __init__(self, value, options:list=[],**a):
        super().__init__(**a)
...
class Range(Tag.div):
    def __init__(self, value,**a):
        ...
        self.input= Tag.input(_type="range",_oninput="this.previousElementSibling.value = this.value", _value=value,**a)
...
class Checkbox(Tag.label, TagBulma):
    def __init__(self, value:bool, label:str,**a):
        super().__init__(**a)
        ...
        self.input=Tag.input(_type="checkbox", _checked = value, **a)
...
class SelectButtons(Tag.div, TagBulma):
    ...
    def __init__(self, value, options:list,**a):
        ...
        self.input = Tag.input(_type="hidden",_value=self.value, **a)
        self.u = Tag.H.ul()
    ...
    class Page(Tag.div):
        ...
        def redraw(self, notUsed ):
            ...
            if self.tab.value==0:
                ...
                f<=Textarea("hello",**commons).named("mytext")
                f<= HBox(
                    Input(2,**commons).named("myinput1"),
                    Input(None,LIST,**commons).named("myinput2"),
                    Input(None,DICT,**commons).named("myinput3"),
                )
                f<= HBox(
                    Input("",_type="date",**commons).named("date"),
                    Input("",_type="month",**commons).named("month"),
                    Input("",_type="time",**commons).named("time"),
                    Input("",_type="password",**commons).named("pass"),
                    Input("",_type="color",**commons).named("color"),
                    Input("",_type="file",**commons).named("file"), # non sense, in a form
                )

                f<= HBox(
                    Radio(2,LIST,**commons).named("myradio1"),
                    Radio("B",DICT,**commons).named("myradio2"),
                )

                f<= HBox(
                    SelectButtons(2,LIST,**commons).named("mysb1"),
                    SelectButtons("B",DICT,**commons).named("mysb2"),
                )

                f<= HBox(
                    Select(2,LIST,**commons).named("myselect1"),
                    Select("B",DICT,**commons).named("myselect2"),
                )

                f<= HBox(
                    Checkbox(True,"homme",**commons).named("mycb1"),
                    Checkbox(False,"femme",**commons).named("mycb2"),
                )

                f<= Range(12, _min=10, _max=78, _step=2, **commons).named("myrange")
                ...
                f.addField("Surname", Input("kookokko",**commons), "pas plus grand que 10")
                f.addField("Sex", Radio(1,[1,2,3],**commons))
                f.addField("Other", [Select(1,[1,2,3],**commons), Select(None,[None,1,2,3],**commons) ])
